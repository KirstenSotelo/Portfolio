...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Sakura animation for hero section
  const createSakura = () => {
    const sakuraContainer = document.getElementById("sakura-container")
    if (!sakuraContainer) return

    const sakuraCount = Math.min(50, Math.floor(window.innerWidth / 20)) // Responsive petal count
  ...
  const createPetal = (container) => {
    ...
    // Randomly flip some petals for variety
    if (Math.random() > 0.5) {
      petal.classList.add("flipped")
    }
    ...
    setTimeout(
      () => {
        petal.remove()
        createPetal(container) // Create a new petal to replace it
      },
      (delay + duration) * 1000,
    )
  }
      ...
      try {
        ...
        if (data.success) {
          ...
          successMessage.innerHTML = `
            <div style="color: var(--success); background: rgba(160, 201, 164, 0.1); padding: var(--space-md); border-radius: var(--border-radius-md); margin-top: var(--space-md);">
        ...
        errorMessage.innerHTML = `
          <div style="color: var(--danger); background: rgba(232, 140, 125, 0.1); padding: var(--space-md); border-radius: var(--border-radius-md); margin-top: var(--space-md);">
  ...
  const addHoverEffects = () => {
    ...
    projectCards.forEach((card, index) => {
      ...
      card.classList.add("sakura-float")
    ...
    skillCards.forEach((card, index) => {
      if (index % 3 === 0) {
        // Apply to every third card for variety
        card.classList.add("sakura-pulse")
      }
    })
  ...
  // Call the skill card mouse effect function
  addSkillCardMouseEffect()

  // Handle window resize for sakura animation
  window.addEventListener("resize", () => {
    const sakuraContainer = document.getElementById("sakura-container")
    if (sakuraContainer) {
      // Clear existing petals
      sakuraContainer.innerHTML = ""
      // Create new petals based on new window size
      createSakura()
    }
  })
})
